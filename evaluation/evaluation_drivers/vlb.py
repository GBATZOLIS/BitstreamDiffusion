...
def evaluate_vlb(args, cfg, model, train_loader, test_loader, device, rank0, run_meta, results_rows):
    ...
    def _get(name, default):
        if vlb_cfg_eval is not None and getattr(vlb_cfg_eval, name, None) is not None:
            return getattr(vlb_cfg_eval, name)
        if vlb_cfg_train is not None and getattr(vlb_cfg_train, name, None) is not None:
            return getattr(vlb_cfg_train, name)
        return default
    ...
    for split in args.vlb_splits:
        ...
        if loader is None:
            if rank0:
                print(f"⚠️  VLB[{split}] requested but loader is not loaded.")
            continue

        for scope in args.vlb_position_scopes:
            res = compute_vlb_over_loader(
                model=model,
                cfg=cfg,
                data_loader=loader,
                device=device,
                sigma_min_eval=sigma_min_eval,
                sigma_max_eval=sigma_max_eval,
                sigma_sampling=sigma_sampling,
                num_mc_samples_per_batch=K,
                include_prior=include_prior,
                use_amp=use_amp,
                amp_dtype=amp_dtype,
                max_batches=args.max_batches,
                progress=rank0,
                allow_conditional_clean_prefix=True,
                position_scope=scope,
            )

            if rank0:
                print(
                    f"✅ VLB[{split}/{scope}]  vlb_bpd={res.vlb_bpd:.4f}  "
                    f"(recon={res.recon_bpd:.4f}, diff={res.diff_bpd:.4f}, "
                    f"prior={res.prior_bpd:.4f})  positions={res.num_positions}  "
                    f"mean_S={res.S_dim}  K={res.K}  dist={res.sigma_sampling}  "
                    f"[σmin={res.sigma_min_eval:g}, σmax={res.sigma_max_eval:g}]"
                )
                results_rows.append(
                    dict(
                        **run_meta,
                        metric=f"vlb_bpd_{scope}",
                        split=split,
                        value=float(res.vlb_bpd),
                        details=(
                            f"scope={scope},positions={res.num_positions},"
                            f"recon={res.recon_bpd:.4f},diff={res.diff_bpd:.4f},"
                            f"prior={res.prior_bpd:.4f},K={res.K},"
                            f"dist={res.sigma_sampling},sigma_min={res.sigma_min_eval},"
                            f"sigma_max={res.sigma_max_eval}"
                        ),
                    )
                )
